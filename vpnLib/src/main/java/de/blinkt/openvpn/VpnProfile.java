...
public class VpnProfile implements Serializable, Cloneable {
    ...
    public static final boolean M_IS_OPEN_VPN_22 = false;
    private static final long SERIAL_VERSION_UID = 7085688938959334563L;
    ...
    public String getConfigFile(Context context, boolean configForOvpn3) {
        ...
        } else if (M_IS_OPEN_VPN_22 && !mUseUdp) {
        ...
        if (usesystemproxy && !M_IS_OPEN_VPN_22 && !configForOvpn3 && !usesExtraProxyOptions()) {
        ...
        if (!canUsePlainRemotes) {
            cfg.append("# Connection Options are at the end to allow global options (and global customdialog options) to influence connection blocks\n");
            for (Connection conn : mConnections) {
                if (conn.mEnabled) {
...
//        }
    ...
    public boolean requireTLSKeyPassword() {
        ...
        if (data.contains("Proc-Type: 4,ENCRYPTED")) {
            ...
            return data.contains("-----BEGIN ENCRYPTED PRIVATE KEY-----");
        }
    }
