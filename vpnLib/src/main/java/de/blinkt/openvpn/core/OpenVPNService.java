...
public class OpenVPNService extends VpnService implements StateListener, Callback, IOpenVPNServiceInternal {
    ...
    private void showNotification() {
        ...
        @SuppressLint("InlinedApi") PendingIntent pe = PendingIntent.getActivity(this, 0, intent, PendingIntent.FLAG_IMMUTABLE);
...
//    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        ThreadUtils.getFixedPool(2).submit(this::startOpenVPN);
...
//                "com.mojang.minecraftpe",
    ...
    private void doSendBroadcast(String state, ConnectionStatus level) {
        ...
        status.setAction("de.blinkt.openvpn.VPN_STATUS");
        status.putExtra("status", level.toString());
        status.putExtra("detailstatus", state);
        sendBroadcast(status, permission.ACCESS_NETWORK_STATE);
        sendMessage(state);
    }
